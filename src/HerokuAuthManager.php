...
class HerokuAuthManager extends OAuth2Manager {
  ...
  /**
   * The Heroku client object.
   *
   * @var \League\OAuth2\Client\Provider\Heroku
   */
  protected $client;
  /**
   * The Heroku access token.
   *
   ...
   * The Heroku user.
   *
   * @var \League\OAuth2\Client\Provider\HerokuUser
   */
  protected $user;
  ...
  public function __construct(LoggerChannelFactoryInterface $logger_factory, EventDispatcherInterface $event_dispatcher, EntityFieldManagerInterface $entity_field_manager, UrlGeneratorInterface $url_generator, ConfigFactory $configFactory) {
    ...
    $this->config             = $configFactory->getEditable('social_auth_heroku.settings');
  }
   ...
   *
   * @return League\OAuth2\Client\Provider\HerokuUser
   *   User info returned by the Heroku.
   */
  ...
  /**
   * Returns the Heroku login URL where user will be redirected.
   *
   * @return string
   *   Absolute Heroku login URL where user will be redirected
   */
  public function getHerokuLoginUrl() {
